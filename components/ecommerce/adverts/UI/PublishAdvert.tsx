...
type PublishAdvertProps = {
  advert: TAdvert["advert"];
};

export const PublishAdvert: React.FC<PublishAdvertProps> = (props) => {
  ...
  const { isPending, mutate } = useMutation({
    mutationFn: advert.publish,
    onSuccess: (response: any) => {
      console.log("publish advert response:", response);
  ...
  const publishAdvertSubmitHandler = (
  ...
  return (
    <View style={styles.container}>
      <Formik
        ...
        onSubmit={(values) => publishAdvertSubmitHandler(values)}
      >
        {(formik) => (
          <View style={styles.formContainer}>
            <View style={styles.publishContentContainer}>
              <Text style={styles.publishContentText}>
                This advert is not yet published. I won't appear among other
                adverts or search results on the platform.
              </Text>
            </View>
